        ...
        <div className="rounded-lg bg-gray-50 p-2 md:pt-0">
          <div className="md:hidden">
            {invoices?.map((invoice) => (
              <MobileTableRow key={invoice.id} invoice={invoice} />
            ))}
          </div>
          <table className="hidden min-w-full text-gray-900 md:table">
            ...
            <tbody className="bg-white">
              {invoices?.map((invoice) => (
                <DesktopTableRow key={invoice.id} invoice={invoice} />
              ))}
            </tbody>
          </table>
        </div>
...
const MobileTableRow = ({ invoice }: { invoice: TInvoicesTable }) => {
  return (
...
const DesktopTableRow = ({ invoice }: { invoice: TInvoicesTable }) => {
  return (
